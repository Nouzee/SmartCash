...
SmartCash 研究使用的权威历史/原始市场数据版本；必须可由 dataset ID、version、数据集内容哈希与实际输入 export 哈希唯一定位。
_Avoid_: Thousand 默认数据源、本地临时 CSV、当前会话缓存

**Beast Transform（Beast 数据变换）**:
由 Beast 所属脚本把冻结的 Vault 数据转换成 SmartCash 规范事件 artifact 的可复现步骤；脚本 commit、配置哈希、输入 export 哈希、到达时间来源与输出哈希必须入 manifest，capture evidence 必须保留 source→derived 双哈希链。
